...
void Removenode_list (Lista plist, nodep node_to_remove){

    lista *list = (lista*) plist;
    node_list  *currNode = (node_list*)node_to_remove;
  ...
  if(currNode->prev == NULL) 
     list->begin = currNode->next;
  else {
    currNode->prev->next =currNode->next;
  }
  if(currNode->next == NULL){
    list->end = currNode->prev; 
  }
  else{
