...
void finaldotgeo(Path p){
    paths* aux = (paths*)p;
    aux->Dotgeo = makedotgeo(aux->Outgeo);
}
...
void finaldotqry(Path p){
    paths* aux = (paths*)p;
    aux->Dotqry = makedotqry(aux->Outqry);
}
...
char *getdotgeo(Path p){
    paths* aux = (paths*)p;
    return aux->Dotgeo;
}

char *getdotqry(Path p){
    paths* aux = (paths*)p;
    return aux->Dotqry;
}
