...
void readArgcArgv(int argc, char **argv,Path p)
{
...
char *saidaDot(char *Outpathconc, char *filename)
{
...
char *makedotgeo(char *outgeo)
{
    int i = strlen(outgeo);
    char *filename = (char *)malloc((i + 5) * sizeof(char));
    filename = saidaDot(outgeo, filename);
...
char *makedotqry(char *outqry)
{
    int i = strlen(outqry);
    char *filename = (char *)malloc((i + 5) * sizeof(char));
    filename = saidaDot(outqry, filename);
    return filename;
}
